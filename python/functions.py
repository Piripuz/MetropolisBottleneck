...
def read_leg_results(directory):
    if USE_PARQUET:
        df = pl.read_parquet(os.path.join(directory, "output", "trip_results.parquet"))
    else:
        df = pl.read_csv(os.path.join(directory, "output", "trip_results.csv"))
    df = df.with_columns(
        (df["exp_arrival_time"] - df["departure_time"]).alias("exp_travel_time"),
        (df["arrival_time"] - df["departure_time"]).alias("travel_time"),
    )
